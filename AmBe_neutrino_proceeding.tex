...
SNO+ reuses most of the SNO infrastructure for calibration in the Water phase.\cite{} The Manipulator system, show schematically in Figure \ref{fig1}, contains a \textit{Umbilical Retrieval Mechanism} (URM) which is mounted on the \textit{Universal Interface} (UI). Different calibration sources can therefore be put in different positions inside the detector, when attched to a 100 ft \textit{Umbilical} and \textit{sideropes}. 
